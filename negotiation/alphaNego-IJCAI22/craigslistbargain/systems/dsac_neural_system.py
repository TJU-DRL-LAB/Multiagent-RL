...
from neural import dsac_model_builder
...
class DSACNeuralSystem(System):
    ...
    """
    def __init__(self, args, schema, price_tracker, model_path, timed, name=None, id=0, dsac_load_dict=None):
        super(DSACNeuralSystem, self).__init__()
        self.args = args
        self.schema = schema
        ...
        model_type = name
        assert model_type == 'pt-neural-dsac'
        ...
        dsac_load_dict = None
        if args.load_type == "from_rl":
            dsac_load_dict ={'actor':args.actor_path, 'zf1':args.zf1_path, 'zf2':args.zf2_path}

            if id == 2:
                dsac_load_dict ={'actor':'checkpoint/dsac_learnsp_seed2_alpha0.1_policylr0_kl0.1_margin_0.8sl_sp/actor_seed2_1100.pth',
                                 'zf1':'checkpoint/dsac_learnsp_seed2_alpha0.1_policylr0_kl0.1_margin_0.8sl_sp/zf1_seed2_500.pth',
                                 'zf2':'checkpoint/dsac_learnsp_seed2_alpha0.1_policylr0_kl0.1_margin_0.8sl_sp/zf2_seed2_500.pth'}
            print("dsac_load_dict", dsac_load_dict)

        mappings, model, model_args = dsac_model_builder.load_test_model(
            model_path, args, dummy_args_dict, new_args, load_type=args.load_type, dsac_load_dict=dsac_load_dict)
        ...
        generator = LFSampler(dsac.actor_model, lf_vocab, 1, max_length=args.max_length, cuda=use_gpu(args), model_type=model_type)
        tom_generator = None
        ...
        Env = namedtuple('Env', ['dsac', 'model','vocab', 'preprocessor', 'textint_map',
            'stop_symbol', 'remove_symbols', 'gt_prefix', 'lfint_map',
            'max_len', 'dialogue_batcher', 'cuda', 'lf_vocab',
            'dialogue_generator', 'utterance_builder', 'model_args', 'usetom',
            'name', 'price_strategy', 'tom_type', 'nlg_module', 'tom_generator', 'tom_model', 'id', 'model_type', 'args'])
        self.env = Env(dsac, dsac, vocab, preprocessor, textint_map,
            ...
            dialogue_generator=generator, utterance_builder=builder, model_args=model_args, usetom=(name == 'tom'), name=name,
            price_strategy=args.price_strategy, tom_type=args.tom_type, nlg_module=nlg_module,
            tom_generator=tom_generator, tom_model=None, id=id, model_type=model_type, args=args)
        # print('usetom?:', (name == 'tom'))

    @classmethod
    def name(cls):
        return 'pt-neural-dsac'
