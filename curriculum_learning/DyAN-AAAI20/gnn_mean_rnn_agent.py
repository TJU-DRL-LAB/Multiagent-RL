...
class GNNMeanRnnAgent(nn.Module):
    def __init__(self, input_shape, args):
        super(GNNMeanRnnAgent, self).__init__()
        self.args = args
    ...
    def forward(self, inputs, hidden_state):
        ...
        # 与max同理，处理之后为0，所以注意下分母即可
        agents_hidden = F.relu(self.agents_info_fc1(agents_feats))
        mean_agents_hidden = th.sum(agents_hidden, dim=1) / (agent_nums.float().reshape(-1, 1) + 1)

        enemies_hidden = F.relu(self.enemies_info_fc1(enemies_feats))
        mean_enemies_hidden = th.sum(enemies_hidden, dim=1) / (enemy_nums.float().reshape(-1, 1) + 1)

        all_feat = th.cat([self_hidden, mean_agents_hidden, mean_enemies_hidden], dim=-1)
        ...
        if self.args.save_agent:
            for idx, value in enumerate(list(agent_nums.detach().cpu().numpy())):
                if value == self.args.see_num:
                    print(mean_agents_hidden.detach().cpu().numpy().tolist()[idx])

        if not self.args.save_agent:
            for idx, value in enumerate(list(enemy_nums.detach().cpu().numpy())):
                if value == self.args.see_num:
                    print(mean_enemies_hidden.detach().cpu().numpy().tolist()[idx])
