...
from envs.MATC_2a2d2t_Room.MATC_Room_with_common_reward import GameEnv
...
parser.add_argument('--meta-mem', type=int, default=10000)
...
parser.add_argument('--before-learning', type=float, default=10000)
parser.add_argument('--train-interval', type=int, default=20)
parser.add_argument('--pre-train', type=int, default=20000)
...
meta_dqn1 = metaDQN(n_features=game_size*game_size*4 + 1,
                    n_actions=goal_num,
...
dqn1 = DQN(n_features=game_size*game_size*5 + 1,
...
meta_dqn2 = metaDQN(n_features=game_size*game_size*4 + 1,
                    n_actions=goal_num,
                    learning_rate=meta_lr,
                    reward_decay=meta_gamma,
                    replace_target_iter=200,
...
dqn2 = DQN(n_features=game_size*game_size*5 + 1,
           n_actions=nav_action_num,
           learning_rate=lr,
           reward_decay=gamma,
           replace_target_iter=200,
...
if IS_SAVE_LOG:
    ...
    writer = tf.summary.FileWriter(logs_path + 'g5g0_lr1_1e11e1_l30fc64b64m10k10kv20_hard200/seed' + str(seed), sess.graph)
#
    ...
    while True:
        ...
        if g1 in range(4):
            ...
            if reached1 or dur1 >= MAX_GOALSTEP:
                ...
                # TODO enhanced experience
                # for i in range(dur1):
                #     meta_dqn1.store_transition(ss1_set[i], g1, sum(af1_set[i:]), dur1 - i, s1_ + [m1_])
        ...
        if g2 in range(4):
            ...
            if reached2 or dur2 >= MAX_GOALSTEP:
                ...
                # TODO enhanced experience
                # for i in range(dur2):
                #     meta_dqn2.store_transition(ss2_set[i], g2, sum(af2_set[i:]), dur2 - i, s2_ + [m2_])
