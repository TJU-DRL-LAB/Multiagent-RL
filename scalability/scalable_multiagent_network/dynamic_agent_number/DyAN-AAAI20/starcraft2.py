...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        # modify: 确定是调用这个smac
        print('#' * 20)
        print("use the smac in project dirs: use grid attack range")
        print('#' * 20)
        ...
        # modify 新的attack space
        self.n_attack = 64

        # modify 原始的action space
        # self.n_actions = self.n_actions_no_attack + self.n_enemies
        self.n_actions = self.n_actions_no_attack + self.n_attack
    ...
    # modify delta_x, delta_y to grid attack action
    def get_attack_grid_id(self, delta_x, delta_y):
        index_x = math.floor(delta_x / 2)
        index_y = math.floor(delta_y / 2)

        if index_x > 3 or index_x < -4 or index_y > 3 or index_y < -4:
            return -1
        ...
        return attack_dir[index_x][index_y]

    def get_attack_emeny_id(self, action, x, y):
        target_grid = action - self.n_actions_no_attack
        attack_unit = None
        attack_e_id = None
        for e_id, e_unit in self.enemies.items():
            # print(e_unit.health > 0)
            # 之前应该是没有检查是否死掉
            if e_unit.health > 0:
                e_x, e_y = e_unit.pos.x, e_unit.pos.y
                delta_x = e_x - x
                delta_y = e_y - y
                attack_grid = self.get_attack_grid_id(delta_x, delta_y)
                if attack_grid == target_grid:
                    if attack_unit is None:
                        attack_unit = e_unit
                        attack_e_id = e_id
                    # 原来是按照血量的
                    elif (e_unit.health + e_unit.shield) < (attack_unit.health + attack_unit.shield):
                    # 现在按照距离
                    # elif math.hypot(delta_y, delta_x) < math.hypot(attack_unit.pos.x - x, attack_unit.pos.y - y):
                        attack_unit = e_unit
                        attack_e_id = e_id
        return attack_unit, attack_e_id
    ...
    def get_agent_action(self, a_id, action):
        ...
        if action == 0:
            ...
            target_unit, target_id = self.get_attack_emeny_id(action, x, y)
            # print(action, target_unit, target_id)
            action_name = "attack"
    ...
    def get_obs_agent(self, agent_id):
        ...
        if unit.health > 0:  # otherwise dead, return all zeros
            ...
            # Enemy features
            # modify
            e_ids = range(len(self.enemies))
            for e_r_id in e_ids:
                e_unit = self.get_enemy_by_id(e_r_id)
                ...
                if (
                    ...
                    # modify 要通过grid来判断是否可以攻击
                    # enemy_feats[e_id, 0] = avail_actions[
                    #     self.n_actions_no_attack + e_id
                    # ]  # available
                    enemy_feats[e_id, 0] = (self.get_attack_grid_id(e_unit.pos.x - unit.pos.x, e_unit.pos.y - unit.pos.y) != -1)
            ...
            for al_id in al_ids:
                ...
                if (
                    ...
                    ally_feats[a_id, 0] = 1  # visible
                    ally_feats[a_id, 1] = dist / sight_range  # distance
                    ally_feats[a_id, 2] = (al_x - x) / sight_range  # relative X
                    ally_feats[a_id, 3] = (al_y - y) / sight_range  # relative Y
                    ...
                    if self.unit_type_bits > 0:
                        ...
                        ally_feats[a_id, ind + type_id] = 1
                        ind += self.unit_type_bits

                    if self.obs_last_action:
                        ally_feats[a_id, ind:] = self.last_action[al_id]
        ...
        agent_obs = np.concatenate(
            (
                move_feats.flatten(),
                enemy_feats.flatten(),
                np.array([e_id], dtype=np.float32),
                ally_feats.flatten(),
                np.array([a_id], dtype=np.float32),
                own_feats.flatten(),
            )
        )
    ...
    def get_state(self):
        ...
        al_ids = range(len(self.agents))
        al_id = 0
        for al_r_id in al_ids:
            al_unit = self.get_unit_by_id(al_r_id)
            if al_unit.health > 0:
                x = al_unit.pos.x
                y = al_unit.pos.y
    ...
    def get_obs_size(self):
        ...
        # modify, + 2: live agent num, live enemy num
        return move_feats + enemy_feats + ally_feats + own_feats + 2
        ...
        if unit.health > 0:
            ...
            for t_id, t_unit in target_items:
                if t_unit.health > 0:
                    ...
                    grid = self.get_attack_grid_id(t_unit.pos.x - unit.pos.x, t_unit.pos.y - unit.pos.y)
                    if grid != -1:
                        avail_actions[grid + self.n_actions_no_attack] = 1
